...
public interface AdministratorRepository extends JpaRepository<Administrator, Integer> {
	...
	@Query("select avg(1*(select count(qu) from Gomela qu where qu.conference.id = q.conference.id)) from Gomela q")
	Double getAvgOfGomelas();

	@Query("select stddev(1*(select count(qu) from Gomela qu where qu.conference.id = q.conference.id)) from Gomela q")
	Double getSDOfGomelas();

	@Query("select 1.0 * count(qf) / (select count(*) from Gomela qd) from Gomela qf where (finalMode = true)")
	Double getRatioOfFinalModeVsTotalGomelas();

	@Query("select 1.0 * count(qf) / (select count(*) from Gomela qd) from Gomela qf where (finalMode = false)")
	Double getRatioOfDraftModeVsTotalGomelas();
