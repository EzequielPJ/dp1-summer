...
public class DashboardAdministratorController extends AbstractController {
	...
	public ModelAndView display() {
		...
		final Double avgOfGomelas = this.administratorService.getAvgOfGomelas();
		if (avgOfGomelas != null)
			result.addObject("avgOfGomelas", avgOfGomelas);
		else
			result.addObject("avgOfGomelas", 0.0);

		final Double sDOfGomelas = this.administratorService.getSDOfGomelas();
		if (sDOfGomelas != null)
			result.addObject("sDOfGomelas", sDOfGomelas);
		else
			result.addObject("sDOfGomelas", 0.0);

		final Double ratioGomelasPublish = this.administratorService.getRatioOfFinalModeVsTotalGomelas();
		if (ratioGomelasPublish != null)
			result.addObject("ratioGomelasPublish", ratioGomelasPublish);
		else
			result.addObject("ratioGomelasPublish", 0.0);

		final Double ratioGomelasUnPublish = this.administratorService.getRatioOfDraftModeVsTotalGomelas();
		if (ratioGomelasUnPublish != null)
			result.addObject("ratioGomelasUnPublish", ratioGomelasUnPublish);
		else
			result.addObject("ratioGomelasUnPublish", 0.0);
